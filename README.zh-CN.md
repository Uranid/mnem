...
**mnem 是 AI Agent 知识的 Git。** 一个面向 AI Agent 的持久化、版本化知识层，在我们测试过的所有公开基准上都取得最佳或并列最佳的召回率。

把源代码、PDF、Markdown 文档、会话导出或整个目录丢进来，mnem 一条命令完成解析、分块和索引。文件类型自动识别：Markdown 按标题分块，多种语言的源代码按函数和类粒度解析，PDF 用滑动窗口抽取文本。摄入过程不调用 LLM，相同输入始终产出相同的图谱；重新摄入未变更的文件是空操作。

技能、决策和约定以节点和类型化边的形式存放在项目 `.mnem/` 目录中的可查询知识图谱里。与代码一起提交，每位队友的 Agent 都从相同的基线起步；像处理源码一样对任何一次写入进行分支、diff、合并或回滚。「遗忘」是一等概念：撤销某条事实后，所有检索路径都会自动将其过滤，并保留审计轨迹。

检索在一次遍历中融合向量搜索、关键词搜索和图遍历，并精确报告消耗了多少 token 以及在预算上限处丢弃了什么；不会有任何内容被静默截断。可选的多跳图扩展能跨多个相关联文档拼接答案。

单一二进制，无需服务器、无需外部数据库、完全离线；同一引擎既是 CLI，也是 HTTP 服务器、MCP 服务器或 Python 库，还能通过 WebAssembly 跑在浏览器里。一条命令即可接入 Claude Code、Cursor、Gemini CLI 或任何 MCP 宿主。开箱即用零配置；当默认 embedder 不够用时，一行配置即可切换提供商。
...
4. [安装](#安装)
5. [快速入门](#快速入门)
6. [接入与取消接入](#mnem-integrate---接入任何-agent-宿主)
7. [命令](#命令)
8. [MCP 工具](#mcp-工具)
9. [Python API](#python-api-mnem-py)
10. [GraphRAG](#graphrag)
11. [你能获得什么](#你能获得什么)
12. [不适合使用 mnem 的场景](#不适合使用-mnem-的场景)
13. [文档](#文档)
14. [贡献](#贡献)
...
- **自动检索触发器** - Claude Code 获得 `UserPromptSubmit` 钩子；Hermes Agent 获得在 `$HERMES_HOME/config.yaml` 中配置的 `pre_llm_call` / `post_llm_call` shell 钩子（当 `$HERMES_HOME` 未设置时默认为 `~/.hermes/config.yaml`）。两者都优先查询本地图谱，然后回退至全局图谱。
- **系统提示** - mnem 使用说明注入拥有规则文件的宿主。Hermes 故意被排除在外，因为其钩子契约设计为以 +1 用户上下文增强而非系统提示修改的方式工作。
