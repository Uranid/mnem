...
pub fn cached_path(bench: Bench) -> Result<PathBuf> {
    let spec = spec_for(bench).ok_or_else(|| {
        anyhow!("no dataset spec for {}", bench.metadata().id)
    })?;
...
pub fn is_cached(bench: Bench) -> bool {
    ...
    sha256_file(&p).map(|h| h.eq_ignore_ascii_case(spec.sha256)).unwrap_or(false)
}
...
pub fn sha256_file(p: &Path) -> Result<String> {
    ...
    let mut buf = vec![0u8; 64 * 1024];
    loop {
        let n = f.read(&mut buf).with_context(|| format!("reading {}", p.display()))?;
        if n == 0 {
            break;
        }
        hasher.update(&buf[..n]);
