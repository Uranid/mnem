...
pub fn run(plan: &RunPlan) -> Result<Vec<BenchOutcome>> {
    ...
    for adapter_kind in &plan.adapters {
        for bench in &plan.benches {
            ...
            let embedder = build_embedder(plan.embedder)
                .map_err(|e| anyhow!("constructing embedder: {e}"))?;
...
fn run_longmemeval(
    ...
    if all.is_empty() {
        bail!("longmemeval dataset at {} contained no questions", path.display());
    }
    if !plan.quiet {
...
fn run_locomo(
    ...
    if all.is_empty() {
        bail!("locomo dataset at {} contained no conversations", path.display());
    }
    if !plan.quiet {
...
fn build_embedder(choice: EmbedderChoice) -> Result<BenchEmbedder> {
    match choice {
        ...
        EmbedderChoice::OnnxMiniLm => {
            ...
            {
                BenchEmbedder::onnx_minilm()
                    .map_err(|e| anyhow!("onnx-minilm init: {e}"))
            }
            #[cfg(not(feature = "onnx-minilm"))]
            {
