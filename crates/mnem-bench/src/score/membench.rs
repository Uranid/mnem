...
pub fn run_simple_roles<A: BenchAdapter>(
    ...
    run(adapter, items, top_k, dataset_path, "mnem-membench-simple-roles")
}
...
fn run<A: BenchAdapter>(
    ...
    for (idx, it) in items.iter().enumerate() {
        pb.inc(1);
        let cat = if it.category.is_empty() { "unknown".to_string() } else { it.category.clone() };
        let topic = if it.topic.is_empty() { "unknown".to_string() } else { it.topic.clone() };
        ...
        let _t = Instant::now();
        adapter.ingest(&docs).map_err(|e| anyhow!("adapter ingest: {e}"))?;
        t_ingest += _t.elapsed().as_secs_f64();

        let limit = top_k.max(10).max(RETRIEVE_LIMIT_DEFAULT.min(docs.len() * 5));
        let _t = Instant::now();
        let hits = adapter
        ...
        let hit_at_k = !target_sids.is_empty()
            && top_sids.iter().any(|s| target_sids.contains(s));
    ...
    let recall_at_k = if total > 0 { hits_total as f64 / total as f64 } else { 0.0 };
