...
pub fn run<A: BenchAdapter>(
    ...
    for q in questions.iter() {
        ...
        for (sid, turns) in q.haystack_session_ids.iter().zip(q.haystack_sessions.iter()) {
            ...
            props.insert("session_id".to_string(), serde_json::Value::String(sid.clone()));
            docs.push(IngestDoc {
        ...
        let hit5 = ranked.iter().take(5).any(|s| answer_set.contains(s.as_str()));
        let hit10 = ranked.iter().take(10).any(|s| answer_set.contains(s.as_str()));

        let qtype_owned = q.question_type.clone().unwrap_or_else(|| "unknown".to_string());
