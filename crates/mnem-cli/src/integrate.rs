...
- **Read**: `mnem global retrieve --text "..." --budget 2000`
...
fn windows_hook_script_content(mnem_bin: &str) -> String {
    ...
    format!(
        "# mnem UserPromptSubmit hook - auto-generated by `mnem integrate`\n\
         ...
         if ($json.prompt) {{\n\
         \x20\x20& '{safe_bin}' global retrieve --text $json.prompt --budget 2000 2>$null\n\
         }}\n"
    )
}
    ...
    #[cfg(not(target_os = "windows"))]
    {
        format!(
            "bash -c 'p=$(jq -r .prompt 2>/dev/null); \
             if [ -n \"$p\" ] && [ \"$p\" != \"null\" ]; then \
             \"{}\" global retrieve --text \"$p\" --budget 2000 2>/dev/null; fi'",
            _mnem_bin.replace('"', "\\\"")
        )
    }
}
