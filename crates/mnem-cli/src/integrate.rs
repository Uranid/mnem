...
pub(crate) enum Host {
    ...
    /// separately in `~/.claude/settings.json` (see [`hooks_path`]).
    ClaudeCode,
...
pub(crate) fn run(args: Args) -> Result<()> {
    ...
    match (args.with_hooks, args.with_system_prompt) {
        ...
        (false, true) => {
            println!(
                "  3. Recommended: also add a guaranteed before-prompt memory hook:"
            );
            println!("       mnem integrate --with-hooks");
        }
        (false, false) => {
            println!(
                "  3. (Recommended) Add the recommended LLM system prompt:"
            );
            println!(
                "       mnem integrate --with-system-prompt        (auto-write, Claude Code)"
            );
            println!(
                "       mnem integrate --system-prompt | clip      (copy-paste, all hosts)"
            );
            println!(
                "  4. (Recommended) Add a guaranteed before-prompt memory hook:"
            );
            println!(
                "       mnem integrate --with-hooks                (Claude Code today)"
            );
        }
    }
    ...
    #[cfg(not(feature = "bundled-embedder"))]
    {
        ...
        println!(
            "      Semantic `mnem retrieve --text` will return zero hits until you configure"
        );
        ...
        .is_some_and(|inner| {
            inner.iter().any(|h| {
                h.get("command")
                    ...
                    .is_some_and(|c| c.contains("mnem retrieve") || c.contains("mnem.exe' retrieve"))
            })
        })
}
...
mod tests {
    ...
    fn system_prompt_constant_is_non_empty_and_mentions_mnem_retrieve() {
        ...
        assert!(SYSTEM_PROMPT.len() > 1000, "system prompt suspiciously small");
    }
    ...
    fn merge_system_prompt_replaces_existing_marker_block_idempotently() {
        let existing = format!(
            "# My project\n\n{}\nOLD PROMPT\n{}\n\n## After mnem section\n",
            SYSTEM_PROMPT_MARKER_START, SYSTEM_PROMPT_MARKER_END
        );
        ...
        assert_eq!(again, out, "second merge with same prompt should be a no-op");
    }
    ...
    fn resolve_mnem_mcp_command_falls_back_to_bare_name_in_test_env() {
        ...
        let ok = cmd == "mnem-mcp"
            || cmd.ends_with("mnem-mcp")
            || cmd.ends_with("mnem-mcp.exe");
