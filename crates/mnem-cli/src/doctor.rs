            ...
            #[cfg(feature = "bundled-embedder")]
            {
                ...
                vec![Check {
                    ...
                    detail: format!(
                        ...
                        c.model,
                        if cache_hit { "present" } else { "not yet - first retrieve will download ~92MB" }
                    ),
                    fix: None,
                }]
            }
...
mod tests {
    ...
    fn adjacent_binary_finds_sibling_with_exe_suffix() {
        ...
        assert_eq!(found.file_name().and_then(|s| s.to_str()), Some(with_suffix.as_str()));
    }
