...
use mnem_embed_providers::{Embedder, EmbedError};
...
fn dataset() -> (Vec<&'static str>, Vec<&'static str>, Vec<(usize, usize)>) {
    let texts: Vec<&'static str> = vec![
        ...
        /* 0 */ "Apple Inc unveiled the iPhone 17 at Cupertino headquarters",
        ...
        /*10 */ "Apple orchards harvest fruit in early autumn",
        ...
        /*20 */ "Banks offer savings accounts with compound interest",
    ...
    let vocab: Vec<&'static str> = vec![
        ...
        "apple", "apples",
        // tech
        "inc", "iphone", "cupertino", "tim", "cook", "ceo", "ipad", "ios",
        "mac", "macbook", "chip", "chips", "developer", "developers", "app",
        "apps", "store", "stock", "watch", "vision", "park", "engineer",
        "engineers", "pro", "headset", "headquarters", "campus", "circular",
        "ring", "launch", "event", "spatial", "computing", "fitness", "heart",
        "rate", "tracks", "thousands", "series", "laptops", "build", "m4",
        "announced", "houses", "surged", "features", "building", "health",
        // fruit
        "orchard", "orchards", "harvest", "fruit", "autumn", "honeycrisp",
        "crunchy", "sweet", "pie", "recipe", "tart", "cinnamon", "gala",
        "fuji", "granny", "smith", "varieties", "pruning", "tree", "trees",
        "yield", "cider", "pressing", "blossom", "blossoms", "pollinator",
        "pollinating", "bee", "bees", "spring", "cold", "cellar", "cellars",
        "fresh", "freshness", "grafting", "rootstock", "washington", "state",
        "produces", "country", "early",
        // finance
        "bank", "banks", "saving", "savings", "account", "accounts", "compound",
        "interest", "credit", "card", "cards", "variable", "apr", "monthly",
        "mortgage", "loan", "loans", "down", "payment", "check", "federal",
        "reserve", "monetary", "policy", "investment", "portfolio", "portfolios",
        "diversify", "asset", "classes", "trading", "volume", "highs", "today",
        "bond", "bonds", "coupon", "maturity", "period", "retirement", "tax",
        "advantaged", "decades", "teller", "tellers", "deposit", "deposits",
        "withdrawal", "withdrawals", "daily", "online", "banking", "wire",
        "transfers", "convenient", "fast", "rates", "market", "fixed",
    ];
...
fn print_vec(idxs: &[usize]) -> String {
    let s: Vec<String> = idxs.iter().map(|i| format!("n{:02}", i)).collect();
    format!("[{}]", s.join(", "))
}
...
fn graphrag_vs_rag_demo() {
    ...
    let truth_groups: Vec<Vec<usize>> = vec![(0..10).collect(), (10..20).collect(), (20..30).collect()];
    ...
    for (gid, tg) in truth_groups.iter().enumerate() {
        ...
        println!("    truth-{}: {}/10 nodes in C{}", ["A", "B", "C"][gid], correct, target);
    }
    let purity = purity_total as f32 / 30.0;
    println!("  partition purity:     {:.2}\n", purity);
    ...
    println!(
        "  GraphRAG:     pull top-{} vector seeds; pick dominant community via Leiden;",
        SEED_POOL
    );
    ...
    for (qid, qtext, truth, label) in &queries {
        ...
        let mut scored: Vec<(usize, f32)> =
            (0..texts.len()).map(|i| (i, cosine(&q_emb, &embs[i]))).collect();
    ...
    println!("== Aggregate ==");
    println!("  mean precision@{}  vector-only:   {:.3}", K, sum_prec_v / n);
    println!("  mean precision@{}  graphRAG:      {:.3}", K, sum_prec_g / n);
    println!("  mean cross-comm entropy  vector:  {:.3}  (mixing bad on polysemy)", sum_div_v / n);
    println!("  mean cross-comm entropy  graph:   {:.3}  (focused = correct)", sum_div_g / n);
    println!();
    ...
    assert!(assignment.community_count() >= 3, "expected >=3 communities");
    assert!(
        purity >= 0.80,
        "partition purity {:.2} below 0.80 threshold",
        purity
    );
    assert!(
...
fn karate_club_with_printed_metrics() {
    ...
    println!("  modularity Q:         {:.4} (Newman 2006 ground truth: Q ~ 0.4)", assignment.modularity);
    println!("  communities found:    {}", assignment.community_count());
    println!("  partition CID:        {}", assignment.content_cid());
    println!("  Mr Hi (node 0)        in community C{:?}", assignment.community_of(make_id(0)).unwrap());
    println!("  John A (node 33)      in community C{:?}", assignment.community_of(make_id(33)).unwrap());
...
fn multihop_graph_expand_demo() {
    ...
    let mut scored: Vec<(usize, f32)> =
        (0..texts.len()).map(|i| (i, cosine(&q, &embs[i]))).collect();
    scored.sort_by(|a, b| b.1.partial_cmp(&a.1).unwrap());
    ...
    println!("\n  GraphRAG 2-hop expansion from seed n{} (\"{}\"):", seed, texts[seed]);
    println!("    1-hop neighbours: {:?}", one_hop.iter().map(|&i| texts[i]).collect::<Vec<_>>());
    println!("    2-hop neighbours: {:?}", two_hop.iter().map(|&i| texts[i]).collect::<Vec<_>>());
    println!(
